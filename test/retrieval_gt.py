...
class StageCounts:
    ...
    #: Query windows the encoder cut before embedding them, and the limit it cut
    #: them at. A truncated query returns a correctly shaped vector for a prefix
    #: of the text, so nothing downstream can tell -- which makes this the one
    #: number that separates "this encoder retrieves worse" from "this encoder
    #: never saw half the query". Absent (``None``) where the provider does not
    #: state a limit.
    queries_truncated: int = 0
    queries_measured_for_truncation: int = 0
    query_sequence_limit: int | None = None
    ...
    #: Outcome per (case, expected term), so two arms can be compared as the paired
    #: measurements they are. Aggregate recall alone forces an unpaired test between
    #: two proportions, which at these corpus sizes cannot resolve the effects the
    #: sweep is looking for -- while the arms in fact differ on the *same* terms.
    term_outcomes: dict[str, bool] = field(default_factory=dict)
    ...
    def observe(
        ...
        *any* expected term survived, which saturates once a case carries several expected
        ...
        if case_id is not None:
            for iri in expected:
                self.term_outcomes[f"{case_id}\t{iri}"] = iri in snapshot_found
        ...
        limit = metrics.get("query_sequence_limit")
        if limit is not None:
            self.query_sequence_limit = int(limit)
            truncated = metrics.get("queries_truncated")
            if truncated is not None:
                self.queries_truncated += int(truncated)
                self.queries_measured_for_truncation += 1
    ...
    def as_dict(self) -> dict[str, Any]:
        """Flat, machine-readable view of the funnel.

        ``render`` is for a person reading one run. A sweep compares hundreds, so
        the same numbers have to leave the process as data -- a printed table that
        has to be re-parsed is how a measurement stops being comparable across arms.

        Returns:
            dict[str, Any]: Rates, raw counters, and the per-ontology breakdown.
        """
        return {
            "cases": self.cases,
            "seed_recall": self.seed_recall,
            "snapshot_recall": self.snapshot_recall,
            "seed_term_recall": self.seed_term_recall,
            "snapshot_term_recall": self.snapshot_term_recall,
            "on_topic_precision": self.on_topic_precision,
            "expected_terms": self.expected_terms,
            "seed_terms": self.seed_terms,
            "snapshot_terms": self.snapshot_terms,
            "graph_stage_net": self.snapshot_terms - self.seed_terms,
            "mean_atoms_after_dedupe": self._mean(self.atoms_after_dedupe),
            "mean_atoms_final": self._mean(self.atoms_final),
            "mean_ontologies_per_case": self._mean(self.ontologies_per_case),
            "mean_snapshot_triples": self._mean(self.snapshot_triple_count),
            "mean_pruned_uris": self._mean(self.snapshot_pruned_uri_count),
            "mean_uri_components": self._mean(self.snapshot_uri_components),
            "empty_snapshots": self.empty_snapshots,
            "queries_truncated": self.queries_truncated,
            "mean_queries_truncated": self._mean(self.queries_truncated),
            "query_sequence_limit": self.query_sequence_limit,
            "seeds_by_ontology": dict(self.seeds_by_ontology),
            "expected_terms_by_ontology": dict(self.expected_terms_by_ontology),
            "seed_terms_by_ontology": dict(self.seed_terms_by_ontology),
            "snapshot_terms_by_ontology": dict(self.snapshot_terms_by_ontology),
            "term_outcomes": dict(self.term_outcomes),
            "starved_ontologies": sorted(
                self.ontologies_with_expected_but_no_seed_terms()
            ),
        }
    ...
    def render(self, title: str) -> str:
        ...
        if self.query_sequence_limit is not None:
            lines.append(
                f"queries truncated         {self.queries_truncated} "
                f"(encoder limit {self.query_sequence_limit} tokens)"
            )
        else:
            lines.append("queries truncated         unknown (provider states no limit)")
