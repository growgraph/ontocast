...
Cumulative effect of the retrieval fixes above, measured on a 20-ontology catalog (160 cases): **seed recall 61.5% → 76.9%**, **snapshot recall 41.5% → 90.6%**. Snapshots grow from a mean 85 to 191 triples, still inside the 550-triple budget, and on-topic precision moves 45.5% → 36.9% — a deliberate trade, since a term absent from the snapshot cannot be used at all while an extra one merely costs context. Both figures come from a corpus of mutually disjoint ontologies (0 cross-ontology schema edges), which overstates both the component-pruning loss and the precision cost relative to a linked stack; the direction is solid, the magnitude is corpus-specific.
...
- **Docs**: align `.env.example` and user-guide defaults for `VECTOR_STORE_*` and `ONTOLOGY_PATCH_*` with `PatchRetrievalConfig` / `VectorStoreConfig` code defaults; document full patch-retrieval parameter table and tuning presets; document snapshot vs catalog and assemble/propose/apply.
