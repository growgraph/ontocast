...
- `CONVERTER_REPAIR_NUMERIC_ARTIFACTS` repairs pattern-local artifacts that the export leaves inside values: the HTML entities `&lt;` `&gt;` `&amp;` `&quot;` `&apos;` (an explicit map, not a general unescape), carriage-return column wraps (`\r` plus inline whitespace before a newline; a blank line after it stays a paragraph break), flattened exponents (`2 × 10 6` → `2 × 10^6`, and a bare `10 6` only behind `~`, `≈`, `∼`, `≃` or "order of"), and single-sided ligature gaps with only one reading (a gap before `ff…`, a gap after a letter-preceded `fi`/`fl`; `the field`, `fl oz`, `cliff face` are left alone). Only the product form reads a **sign**: `3 × 10 −3` becomes `3 × 10^-3`, while `∼10−15` is left as written, because a publisher typesets a numeric range with the same minus-sign character a negative exponent uses and behind a bare cue the range is the commoner reading -- leaving a real bare negative exponent alone costs nothing downstream, rewriting a range destroys the value invisibly. It does **not** touch superscript/subscript duplication, citation markers fused into a value, or a gap before `fi`/`fl` -- those need layout evidence the exported text no longer carries. Off by default and not part of `born_digital`; it composes with `CONVERTER_REPAIR_LIGATURE_GAPS` (the two-sided rule runs first) and joins the converter cache key only when on, together with a repair-rules version so that a corrected rule re-converts instead of being served text the old rule produced. Every conversion cached with repair off stays cached.
...
# VECTOR_STORE_PROPOSITION_WINDOW_MAX_TOKENS=  # encoder-token bound; replaces both
# VECTOR_STORE_PROPOSITION_WINDOW_OVERLAP=0.0  # fractional overlap under a budget
# VECTOR_STORE_PROPOSITION_ABBREVIATION_AWARE=false
# VECTOR_STORE_PROPOSITION_MEASUREMENT_AWARE=false
...
| `VECTOR_STORE_PROPOSITION_WINDOW_MAX_TOKENS` | unset | Encoder word pieces per window; **replaces** both bounds above when set. The only bound stated in the unit the encoder truncates on, so below the sequence limit it rules truncation out by construction — and the only one that cuts an over-long sentence (at whitespace) rather than handing it to the encoder to cut. Needs a provider with a tokenizer; without one it approximates by characters and logs that it did |
| `VECTOR_STORE_PROPOSITION_WINDOW_OVERLAP` | `0.0` | Fraction of a window repeated at the start of the next, under a character or token budget. `VECTOR_STORE_PROPOSITION_WINDOW_STRIDE` is the sentence-granular spelling and does not apply under a budget. Overlap multiplies queries, and once `PROPOSITION_MAX_WINDOWS` binds it buys them by dropping coverage elsewhere |
| `VECTOR_STORE_PROPOSITION_ABBREVIATION_AWARE` | `false` | Rejoin fragments the period-splitter created inside an abbreviation, an initial or a citation run. General English and bibliographic shapes only — never a domain vocabulary. Off by default: it changes the windows every existing measurement was taken on |
| `VECTOR_STORE_PROPOSITION_MEASUREMENT_AWARE` | `false` | Forbid a break between a number and its unit, or inside a range, from the number/unit *shapes* in the shared measurement lexicon. Only binds where a cut inside a sentence is possible, i.e. under a token budget |
...
| `VECTOR_STORE_WIPE_ON_INIT` | `false` | Drop the current tenant/project vector partition before recreate+reindex (clean slate; also CLI `--wipe-vector-store`). The drop runs whenever a vector store is configured; the recreate+reindex needs `ONTOLOGY_CONTEXT_MODE=selected_vector_search_ontology` |
