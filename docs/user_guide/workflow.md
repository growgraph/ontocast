...
| `FACTS_CRITIC_PASSES` | Review-and-patch passes per facts unit, **in provider calls**. Each pass re-runs the deterministic checks for free, then buys one critique and applies it. `0` is extraction only. See [Validation](validation.md#how-many-llm-calls-a-facts-unit-really-costs) |
