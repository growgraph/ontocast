...
def crawl_directories(
    ...
    Returns:
        Matching paths; empty when a directory holds none.

    Raises:
        ValueError: If ``input_path`` does not exist, or names a file whose
            suffix is not in ``suffixes``. Callers surface this to the user --
            returning an empty list here would read as "nothing to do" and exit
            cleanly, which is what made a mistyped path silently do nothing.
    """
    file_paths: list[pathlib.Path] = []

    if input_path.is_file():
        if input_path.suffix not in suffixes:
            raise ValueError(
