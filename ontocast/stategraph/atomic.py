...
def _resolve_retry_limit(state_retries: int, override: int | None) -> int:
    """Return a safe retry limit while respecting explicit overrides."""
    retries = state_retries if override is None else override
...
async def facts_loop(
    state: UnitFactsState, tools: AtomicToolBox, max_retries: int | None = None
) -> UnitFactsState:
    ...
    retries = _resolve_retry_limit(unit_state.max_retries, max_retries)

    for render_attempt in range(1, retries + 1):
        unit_state.node_visits[WorkflowNode.TEXT_TO_FACTS] = render_attempt - 1
        ...
        for critic_attempt in range(1, retries + 1):
            unit_state.node_visits[WorkflowNode.CRITICISE_FACTS] = critic_attempt - 1
...
async def ontology_loop(
    state: UnitOntologyState, tools: AtomicToolBox, max_retries: int | None = None
) -> UnitOntologyState:
    ...
    for render_attempt in range(1, retries + 1):
        unit_state.node_visits[WorkflowNode.TEXT_TO_ONTOLOGY] = render_attempt - 1
        ...
        for critic_attempt in range(1, retries + 1):
            unit_state.node_visits[WorkflowNode.CRITICISE_ONTOLOGY] = critic_attempt - 1
