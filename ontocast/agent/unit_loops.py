...
async def run_unit_facts_loop(
    ...
    agent_state = unit_state.to_agent_state()

    for attempt in range(1, retries + 1):
        agent_state = await render_facts(agent_state, tools)
        if agent_state.status != Status.SUCCESS:
            logger.info(f"Unit facts render failed at attempt {attempt}/{retries}")
            continue

        agent_state = await criticise_facts(agent_state, tools)
        if agent_state.status == Status.SUCCESS:
            logger.info(f"Unit facts loop converged at attempt {attempt}/{retries}")
            unit_state.apply_agent_result(agent_state)
            return unit_state
...
async def run_unit_ontology_loop(
    ...
    for attempt in range(1, retries + 1):
        agent_state = await render_ontology(agent_state, tools)
        if agent_state.status != Status.SUCCESS:
            logger.info(f"Unit ontology render failed at attempt {attempt}/{retries}")
            continue

        agent_state = await criticise_ontology(agent_state, tools)
        if agent_state.status == Status.SUCCESS:
